...
DECLARE_LOG_CATEGORY_EXTERN(LogNotera, Log, All);
...
{
	Edc_Notera		UMETA(DisplayName = "Notera"),
	Edc_Others  UMETA(DisplayName = "Others")
};

UCLASS(Category = "Notera")
class NOTERA_API UDebugUtil : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
#define PRINT_DEBUG(FormatMsg, ...) \
	UDebugUtil::PrintDebugMessage(FString::Printf(TEXT(FormatMsg), ##__VA_ARGS__), EDebugCategories::Edc_Notera, FLinearColor::White, 5.f, __FUNCTION__, __LINE__)
...
#define PRINT_DEBUG_WARNING(FormatMsg, ...) \
	UDebugUtil::PrintDebugMessage(FString::Printf(TEXT(FormatMsg), ##__VA_ARGS__), EDebugCategories::Edc_Notera, FLinearColor::Red, 5.f, __FUNCTION__, __LINE__, true)
