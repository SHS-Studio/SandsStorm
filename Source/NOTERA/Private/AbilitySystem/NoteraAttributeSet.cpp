...
void UNoteraAttributeSet::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
...
void UNoteraAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	if(Attribute == GetHealthAttribute() && GetHealth() <= 0.f)
	{
		...
		UAbilitySystemBlueprintLibrary::SendGameplayEventToActor(Data.EffectSpec.GetEffectContext().GetInstigator(), NoteraTags::Events::KillScored, Payload);
	}
...
void UNoteraAttributeSet::OnRep_AttributesInitialized()
{
	if(bAttributesInitialized)
		OnAttributesInitialized.Broadcast();
}

void UNoteraAttributeSet::PostAttributeInitialized()
{
