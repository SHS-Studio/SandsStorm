...
void UNoteraAbilitySystemComponent::OnGiveAbility(FGameplayAbilitySpec& AbilitySpec)
{
...
void UNoteraAbilitySystemComponent::OnRep_ActivateAbilities()
{
...
void UNoteraAbilitySystemComponent::SetAbilityLevel(const TSubclassOf<UGameplayAbility> AbilityClass, const int32 Level)
{
...
void UNoteraAbilitySystemComponent::AddToAbilityLevel(const TSubclassOf<UGameplayAbility> AbilityClass, const int32 LevelsToAdd)
{
	if(IsValid(GetAvatarActor()) && !GetAvatarActor()->HasAuthority()) return;
...
void UNoteraAbilitySystemComponent::HandleAutoActivatedAbility(const FGameplayAbilitySpec& AbilitySpec)
{
	...
	for(const FGameplayTag& Tag : AbilitySpec.Ability->GetAssetTags())
		if(Tag.MatchesTagExact(NoteraTags::NoteraAbilities::ActivateOnGiven))
		{
