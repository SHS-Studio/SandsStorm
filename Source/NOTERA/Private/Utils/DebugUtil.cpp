...
// Log categories
DEFINE_LOG_CATEGORY(LogNotera);
DEFINE_LOG_CATEGORY(LogOthers);

// Console variables acting as runtime debug toggles
static TAutoConsoleVariable CVarDebugNotera(TEXT("Notera.Debug"), true, TEXT("Master Debug Toggle for Notera Course"));
static TAutoConsoleVariable CVarDebugPrintToScreen(TEXT("Notera.Debug.PrintToScreen"), true, TEXT("Enable/Disable Print To Screen"));
static TAutoConsoleVariable CVarDebugPrintToLog(TEXT("Notera.Debug.PrintToLog"), true, TEXT("Enable/Disable Print To Log"));
static TAutoConsoleVariable CVarDebugOthers(TEXT("Notera.Debug.Others"), true, TEXT("Enable other debug prints"));
...
FColor UDebugUtil::GetColorForCategory(const EDebugCategories Category, const FLinearColor Default)
{
	switch(Category)
	{
		case EDebugCategories::Edc_Notera : return FColor::Yellow;
...
{
	// Master toggle
	if(!CVarDebugNotera->GetBool())
		return false;

	switch(Category)
	{
		case EDebugCategories::Edc_Notera : return true;
...
void UDebugUtil::PrintDebugMessage(const FString& Msg, const EDebugCategories Category, const FLinearColor LinearColor, const float Time, const char* FunctionName, const int Line, const bool bWarning)
{
	...
	if(CVarDebugPrintToLog->GetBool() || bWarning)
	{
		switch(Category)
		{
			case EDebugCategories::Edc_Notera :
				UE_LOG(LogNotera, Log, TEXT("%s"), *FinalMsg);
				break;
