...
EHitDirection UNoteraBlueprintLibrary::GetHitDirection(const FVector& TargetForward, const FVector& ToInstigator)
{
...
FName UNoteraBlueprintLibrary::GetHitDirectionName(const EHitDirection& HitDirection)
{
	switch(HitDirection)
	{
...
FClosestActorWithTagResult UNoteraBlueprintLibrary::FindClosestActorWithTag(const UObject* WorldContextObject, const FVector& Origin, const FName& Tag, const float SearchRange)
{
	...
	for(AActor* Actor : ActorsWithTag)
	{
		...
		// ReSharper disable once CppTooWideScopeInitStatement
		const ANoteraBaseCharacter* BaseCharacter = Cast<ANoteraBaseCharacter>(Actor);
...
void UNoteraBlueprintLibrary::SendDamageEventToPlayer(AActor* Target, const TSubclassOf<UGameplayEffect>& DamageEffect, FGameplayEventData& Payload, const FGameplayTag& DataTag, const float Damage, const FGameplayTag& EventTagOverride, UObject* OptionalParticleSystem)
{
	ANoteraBaseCharacter* PlayerCharacter = Cast<ANoteraBaseCharacter>(Target);
	...
	if(!EventTagOverride.MatchesTagExact(NoteraTags::None))
		EventTag = EventTagOverride;
	else
	{
		const UNoteraAttributeSet* Attr = Cast<UNoteraAttributeSet>(PlayerCharacter->GetAttributeSet());
		if(!IsValid(Attr)) return;

		const bool bLethal = Attr->GetHealth() - Damage <= 0.f;
		EventTag = bLethal ? NoteraTags::Events::Player::Death : NoteraTags::Events::Player::HitReact;
...
void UNoteraBlueprintLibrary::SendDamageEventToPlayers(TArray<AActor*> Targets,const TSubclassOf<UGameplayEffect>& DamageEffect, FGameplayEventData& Payload, const FGameplayTag& DataTag, const float Damage, const FGameplayTag& EventTagOverride, UObject* OptionalParticleSystem)
{
	for(AActor* Target : Targets)
		SendDamageEventToPlayer(Target, DamageEffect, Payload, DataTag, Damage, EventTagOverride, OptionalParticleSystem);
}

TArray<AActor*> UNoteraBlueprintLibrary::HitBoxOverlapTest(AActor* AvatarActor, float HitBoxRadius, float HitBoxForwardOffset, float HitBoxElevationOffset, bool bDrawDebugs)
{
	...
	{
		// ReSharper disable once CppTooWideScopeInitStatement
		ANoteraBaseCharacter* HitChar = Cast<ANoteraBaseCharacter>(Result.GetActor());
...
void UNoteraBlueprintLibrary::DrawHitBoxOverlapDebugs(const UObject* WorldContextObject, const TArray<FOverlapResult>& OverlapResults, const FVector& HitBoxLocation, const float HitBoxRadius)
{
...
TArray<AActor*> UNoteraBlueprintLibrary::ApplyKnockback(AActor* AvatarActor, const TArray<AActor*>& HitActors, const float InnerRadius, const float OuterRadius, const float LaunchForceMagnitude, const float RotationAngle, const bool bDrawDebugs)
{
	for(AActor* HitActor : HitActors)
	{
		...
		if(ANoteraEnemyCharacter* Enemy = Cast<ANoteraEnemyCharacter>(HitCharacter); IsValid(Enemy))
